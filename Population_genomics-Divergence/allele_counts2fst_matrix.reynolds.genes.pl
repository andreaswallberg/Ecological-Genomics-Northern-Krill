...
sub read_gff {
    ...
    $opt->{ 'app' }->{ 'flanking_table' } = $flanking_table;
    
    my $gene_table = {};
    
    $opt->{ 'app' }->{ 'gene_table' } = $gene_table;
    
    foreach my $file ( @{ $opt->{ 'gff' } } ) {
    
        say $file;
    
        my ( $in ) = get_filehandle( $file , "GFF" );
        
        # seq_c_102672	transdecoder	gene	4169	13188	.	+	.	ID=COM_SPAL_1_000055_XLOC_000252;Name=ORF%20type%3A3prime_partial%20len%3A1431%20%28%2B%29%2Cscore%3D268.22%2Csp%7CQ5GH73%7CXKR6_HUMAN%7C30.952%7C1.92e-44%2CXK-related%7CPF09815.11%7C5e-76
    
        while (<$in>) {
        
            chomp;
            
            if ( $_ =~ m/^(\S+)\s\S+\sgene\s(\d+)\s(\d+)\s\S+\s(\S)\s.+ID\=([^\;]+)\;/ ) {
            
                my ( $chrom , $start , $stop , $orient , $gene ) = ( $1 , $2 , $3 , $4 , $5 );
                
                $gene_table->{ $chrom }->{ $gene }->{ 'chrom' } = $chrom;
                $gene_table->{ $chrom }->{ $gene }->{ 'orient' } = $orient;
                $gene_table->{ $chrom }->{ $gene }->{ 'start' } = $start;
                $gene_table->{ $chrom }->{ $gene }->{ 'stop' } = $stop;
                $gene_table->{ $chrom }->{ $gene }->{ 'gene' } = $gene;
                
                my $before_start = $start - 100_000;
                my $before_stop = $start - 50_000;
                
                my $relation = "upstream";
                ...
                $flanking_table->{ $chrom }->{ $gene . "|" . $relation }->{ 'chrom' } = $chrom;
                $flanking_table->{ $chrom }->{ $gene . "|" . $relation }->{ 'orient' } = $orient;
                $flanking_table->{ $chrom }->{ $gene . "|" . $relation }->{ 'start' } = $before_start;
                $flanking_table->{ $chrom }->{ $gene . "|" . $relation }->{ 'stop' } = $before_stop;
                $flanking_table->{ $chrom }->{ $gene . "|" . $relation }->{ 'gene' } = $gene;
                
                my $after_start = $stop + 50_000;
                my $after_stop = $stop + 100_000;
                
                my $relation = "downstream";
                ...
                $flanking_table->{ $chrom }->{ $gene . "|" . $relation }->{ 'chrom' } = $chrom;
                $flanking_table->{ $chrom }->{ $gene . "|" . $relation }->{ 'orient' } = $orient;
                $flanking_table->{ $chrom }->{ $gene . "|" . $relation }->{ 'start' } = $after_start;
                $flanking_table->{ $chrom }->{ $gene . "|" . $relation }->{ 'stop' } = $after_stop;
                $flanking_table->{ $chrom }->{ $gene . "|" . $relation }->{ 'gene' } = $gene;
...
sub parse_alleles {
    ...
    my $flanking_table = $opt->{ 'app' }->{ 'flanking_table' };
            ...
            while (<$alleles_in>) {
                ...
                if ( $_ =~ m/^(\S+)\s(\S+)\s(\S+)\s(\S+)\s(\S+)\s(.+)/ ) {
                    ...
                    if (
                        ...
                        defined $last_gene and
                        $pos >= $last_gene->{ 'start' } and
                        $pos <= $last_gene->{ 'stop' }
                        ...
                        foreach my $gene ( keys %{ $flanking_table->{ $chrom } } ) {
                        
                            if (
                            
                                $pos >= $flanking_table->{ $chrom }->{ $gene }->{ 'start' } and
                                $pos <= $flanking_table->{ $chrom }->{ $gene }->{ 'stop' }
                            
                            ) {
                            
                                my $tmp_gene = $flanking_table->{ $chrom }->{ $gene }->{ 'gene' };
                            
                                $last_gene = $gene_table->{ $chrom }->{ $tmp_gene };
                        ...
                        foreach my $gene ( keys %{ $gene_table->{ $chrom } } ) {
                        
                            if (
                            
                                $pos >= $gene_table->{ $chrom }->{ $gene }->{ 'start' } and
                                $pos <= $gene_table->{ $chrom }->{ $gene }->{ 'stop' }
                            
                            ) {
                            
                                $last_gene = $gene_table->{ $chrom }->{ $gene };
					...
					for ( my $i = 0; $i < @groups - 1; $i++ ) {
						...
						for ( my $j = $i + 1; $j < @groups; $j++ ) {
							...
							if ( $n1 and $n2 ) {
								...
								if ( $opt->{ 'gff' } and defined $last_gene ) {
                                    ...
                                    foreach my $tmp_symbol ( @symbols ) {
                                    
                                        $last_gene->{ 'fst' }->{ $tmp_symbol }->
                                        { $contrast }->
                                        { 'numerator' } += $numerator;
    ...
    if ( keys %{ $gene_table } and $opt->{ 'gff' } ) {
...
sub print_genes {
    ...
    my @symbols = sort keys %{ $opt->{ 'region' } };
    ...
    foreach my $contrast ( @sorted_contrasts ) {
        ...
        open (
            ...
            my $genes_out ,
            ">" ,
            $output . ".fst.reynolds.genes.${contrast}.csv"
        ) or die  "Unable to gene FST file: $!";
    
        print $genes_out "NR\tGENE\tSTART\tSTOP\tLENGTH";
    
        foreach my $symbol ( @symbols ) {
        
            my $region = $opt->{ 'region' }->{ $symbol };
        
            print $genes_out "\t${region}_FST\t${region}_N";
        ...
        foreach my $chrom ( @sorted_chromosomes ) {
            ...
            if ( defined $gene_table->{ $chrom } ) {
                ...
                my @sorted_genes = sort {
                    $gene_table->{ $chrom }->{ $a }->{ 'start' } <=>
                    $gene_table->{ $chrom }->{ $b }->{ 'stop' }
                } keys %{ $gene_table->{ $chrom } };
                
                foreach my $gene ( @sorted_genes ) {
                
                    my $this_gene = $gene_table->{ $chrom }->{ $gene };
                    
                    print $genes_out "$i\t" , $gene , "\t" ,
                    $this_gene->{ 'start' } , "\t" ,
                    $this_gene->{ 'stop' } , "\t" , 
                    $this_gene->{ 'stop' } - $this_gene->{ 'start' } + 1;
                    ...
                    foreach my $symbol ( @symbols ) {
                        ...
                        if ( defined $this_gene->{ 'fst' }->{ $symbol }->{ $contrast } ) {
                        
                            $fst =
                            $this_gene->{ 'fst' }->{ $symbol }->{ $contrast }->{ 'numerator' } /
                            $this_gene->{ 'fst' }->{ $symbol }->{ $contrast }->{ 'denominator' };
                            
                            $n =
                            $this_gene->{ 'fst' }->{ $symbol }->{ $contrast }->{ 'n' };
                            
                        }
                        
                        print $genes_out "\t" , $fst , "\t" , $n;
