	...
	foreach my $repeat ( @repeats ) {
		...
		if (
			...
			unless (
				...
				$ltr_table->{ $repeat }->{ 'RepeatClassifier' }->{ 'type' } =~ m/^RC/ or
		...
		if (
			...
			defined $ltr_table->{ $repeat }->{ 'RepeatClassifier' }->{ 'type' }
			and
			$ltr_table->{ $repeat }->{ 'RepeatClassifier' }->{ 'type' } =~ m/^RC/i
			...
			and
			$ltr_table->{ $repeat }->{ 'TEsorter' }->{ 'type' } =~ m/^RC/i
			...
			and
			$ltr_table->{ $repeat }->{ 'TransposonPSI' }->{ 'type' } =~ m/^RC/i
